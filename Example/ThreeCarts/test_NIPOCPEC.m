...
clear all
clc
delete ThreeCarts.gif
delete Gen_InitialGuess.mat
[~, ~, ~] = rmdir('autoGen_CodeFiles', 's');

%% create an dynamics system 
timeStep = 0.04;
plant = ThreeCarts(timeStep, [], [], []);
tau_Max = [20; 20; 20];
tau_Min = [-20; -20; -20];
x_Max = [10; 10; 10; 5; 5; 5];
x_Min = [-10; -10; -10; -5; -5; -5];
plant.setDynVarLimit(tau_Max, tau_Min, x_Max, x_Min) ;
plant.codeGen();

%% formulate an OCPEC problem 
nStages = 100;
InitState = [-3; 0; 3; 0; 0; 0];
StageCost.xRef = repmat([-5; 0; 5; 0; 0; 0], 1, nStages);
StageCost.tauRef = repmat([0; 0; 0], 1, nStages);
StageCost.xWeight = [10; 1; 10; 0.1; 0.1; 0.1];
StageCost.tauWeight = [0.1; 0.1; 0.1]; 
TerminalCost.xRef = [-5; 0; 5; 0; 0; 0];
TerminalCost.tauRef = [0; 0; 0];
TerminalCost.xWeight = [100; 10; 100; 1; 1; 1];
TerminalCost.tauWeight = [0.1; 0.1; 0.1];
