function [z_Opt, Info] = solveNLP_IP_single(OCPEC, NLP, solver, z_Init, s_Init, s_End)
...
kappa_s_times = 0.9;
kappa_s_exp = 1.1;
% evaluate number of parameter sequence based on s_Init and s_End
s_test = s_Init;
paramNum = 1;
while true
    if s_test == s_End
...
Log.param      = zeros(paramNum, 1); % s
Log.cost       = zeros(paramNum, 1);
Log.KKT_error  = zeros(paramNum, 3); % [primal, dual_scaled, total]
Log.VI_nat_res = zeros(paramNum, 1);
Log.iterNum    = zeros(paramNum, 1);
Log.time       = zeros(paramNum, 1); % elapsed time in each step

%% solve each NLP with cold start
s_j = s_Init;
timeElasped = 0;
for j = 1 : paramNum
    % step 1: solve a NLP with given s
    timeStart_j = tic;
    solution_j = solver('x0', z_Init, 'p', s_j,...
    ...
    % step 2: record and print information of the current iterate
    ...
    prevIterMsg = ['  ',...
        num2str(j,'%10.2d'), '/', num2str(paramNum,'%10.2d'),'   | ',...
